"""
Starlight System - Serial Manager
===================================

Discovers serial ports that match the naming patterns used by USB-connected
ESP32 devices on macOS and opens a SerialConnection for each one. A background
scanner thread periodically checks for new ports so receivers can be hot-plugged
without restarting the controller.

Port identification strategy
-----------------------------
Rather than relying on device-name heuristics alone, the system uses the
*heartbeat protocol* for authoritative receiver identification: any device that
speaks valid heartbeat JSON is registered as a receiver. Devices that do not
send heartbeats are simply ignored by the controller's message dispatcher, so
connecting to an unexpected port is harmless.
"""
...
# CP210x, CH340, and FTDI chipsets all appear under /dev/cu.* on macOS.
...
class SerialManager:
    ...
    All connections share a single output queue (the controller's message queue)
    so the controller never needs to poll individual connections.
    ...
    def __init__(self, baud_rate: int, shared_queue: queue.Queue):
        ...
        # port path → SerialConnection instance
    ...
    def _discover_and_connect(self):
        """
        Enumerate ports matching the ESP32 USB patterns and open connections
        for any ports not yet tracked.
        """
        discovered: set = set()
